...
class XGngeo:
   ...
   def rom_list(self, widget):
      ...
      def new_list_selection(*args):
            ...
            if iter:  # Is there anything selected?
               ...
               self.mamename = mamename  # Exporting current selected mamename.
      ...
      # Setting scrolled window's height.
      scrolled_window.set_size_request(500, 250)
      ...
      if callback:
         if type(callback) == str:
            def response(dialog, response):
               ...
               """
               if response == gtk.RESPONSE_OK:
                  self.widgets["config"][callback]\
                     .set_text(dialog.get_filename())
      ...
      if rompreview:
         ...
         def scan_dir_for_roms(*args):
            ...
            self.params["temp"]["currdir_romscaninfos"] = \
               self.emulator.scan_rom_in_directory(
                  self.widgets["fileselect_dialog"].get_current_folder(), True)
         ...
         def update_preview(*args):
            ...
            if selection and os.path.isfile(selection): 
               ...
               if self.params["temp"]["currdir_romscaninfos"].has_key(filename):
                  # Current selection has been detected as a ROM:
                  # updating preview!
      ...
      if os.path.exists(path):  # ROM exists on file system, continuing...
         ...
         if archive_infos:
            ...
            dialog.format_secondary_text(_("Such an issue has two "
               "explanations: since the time it ran fine, the ROM archive "
               "has been corrupted or the driver that used to handle this "
               "ROM has been altered/deleted (most probable)."))
            dialog.connect("response", lambda *args: dialog.destroy())
            dialog.show_all()
         ...
         if availability:
            # Since a ROM marked as ``available" wasn't actually found,
            # refreshing the entire ROM History list for the sake of accuracy.
            self.history.refresh_list(size=int(self.params["xgngeo"]\
               ["historysize"]))
            self.history_menu_generation()
            message += "\n%s" % _("It is now indicated as unavailable in the "
               "ROM History menu.")
      ...
      if response == gtk.RESPONSE_OK:
         ...
         if os.path.isfile(path): # Does that file exist?
            ...
            if archive_infos:
               ...
               dialog = gtk.MessageDialog(flags=gtk.DIALOG_MODAL,
                  type=gtk.MESSAGE_ERROR, buttons=gtk.BUTTONS_OK,
                  message_format = _("This ROM is unloadable because GnGeo "
                  "could not find any suitable driver to handle it."))
               dialog.format_secondary_text(_("There are several possible "
                  "reasons for that problem: the archive you wanted to load "
                  "may not be a Neo Geo ROM (most probable), or it may be a "
                  "weird dump, or finaly it may be a rare/new game which isn't "
                  "supported yet.\nFor the 2 last causes, the solution is to "
                  "enlarge the GnGeo ROM driver library in order to make the "
                  "emulator being able to play more ROMs."))
               dialog.connect("response", lambda *args: dialog.destroy())
               dialog.show_all()
            ...
            dialog = gtk.MessageDialog(flags=gtk.DIALOG_MODAL,
               type=gtk.MESSAGE_ERROR, buttons=gtk.BUTTONS_OK,
               message_format=_("Error: file doesn't exist!"))
            dialog.connect("response", lambda *args: dialog.destroy())
            dialog.show_all()
   ...
   def history_menu_generation(self):
      ...
      for val in self.history.get_list():
         ...
         menu_item.connect("button_press_event", self.history_item_clicked,
            val[0], val[1], val[2], pos)
   ...
   def config(self, widget = None, type = 0, firstrun = 0, romspecific = 0):
      def set_path_icon(widget, image, dir = 0, special = None):
         ...
         if not special:
            ...
            val = self.get_bios_presence(path)
            if val:
               stock = 1
      ...
      if firstrun:
         self.widgets["config"]["main_dialog"].connect("delete_event",
            self.quit)
      ...
      if type == 0:
         ...
         # 320x224 window size.
         self.widgets["config"]['screen320'] = gtk.CheckButton(
            _("Larger screen"))
         ...
         i=0; effect_list = []
         pipe = os.popen('"%s" --effect help' % self.params["xgngeo"]\
            ["gngeopath"].replace('"', '\"'))
         for line in pipe.readlines():
            plop = match("(\S*)\s*:(.*)", line) # Syntax is "REF : FULLNAME"
            if plop:
         ...
         def bouyaka(*args):
            """Overlay does not support effect. So, when this blitter is
               selected, we set the effect to ``none" and prevent it from being
               changed by user.
               Effects are also not (yet) supported when enabling hardware
               surface.
         ...
         # Performing special actions over effect widget regarding blitter
         # selection or hardware surface activation.
         bouyaka()
         ...
         for x in range(4): self.widgets["config"]['p1joydev'].append_text(
            "/dev/js%s" % x)
         # Setting active the last selection
         self.widgets["config"]['p1joydev'].set_active(int(
            temp_param["p1joydev"])) 
         ...
         for x in range(4): self.widgets["config"]['p2joydev'].append_text(
            "/dev/js%s" % x)
         # Setting active the last selection
         self.widgets["config"]['p2joydev'].set_active(int(
            temp_param["p2joydev"])) 
         ...
         compliant_keymap = {
            "backspace": 8, "tab":9, "return":13, "pause": 19, "space": 32,
            "exclam": 33, "quotedbl": 34, "dollar": 36, "ampersand": 38,
            "apostrophe": 39, "parenleft": 40, "parenright": 41, "comma": 44,
            "minus": 45, "colon": 58, "semicolon":59, "less":60, "equal": 61,
            "asciicircum": 94, "underscore": 95, "a": 97, "b": 98, "c": 99, "d":
            100, "e": 101, "f": 102, "g": 103, "h": 104, "i": 105, "j": 106,
            "k": 107, "l": 108, "m": 109, "n": 110, "o": 111, "p": 112, "q":
            113, "r": 114, "s": 115, "t": 116, "u": 117, "v": 118, "w": 119,
            "x": 120, "y": 121, "z": 122, "delete": 127, "twosuperior": 178,
            "agrave": 224, "ccedilla": 231, "egrave": 232, "eacute": 233,
            "ugrave": 249, "kp_0": 256, "kp_1": 257, "kp_2":258, "kp_3":259,
            "kp_4": 260, "kp_5": 261, "kp_6": 262, "kp_home": 263, "kp_7": 263,
            "kp_up": 264, "kp_8": 264, "kp_9": 265, "kp_decimal": 266,
            "kp_divide":267, "kp_multiply": 268, "kp_subtract": 269, "kp_add":
            270, "kp_enter": 271, "up": 273, "down": 274, "right": 275, "left":
            276, "insert": 277, "home": 278, "end": 279, "page_up": 280,
            "page_down": 281, "num_lock": 300, "caps_lock": 301, "scroll_lock":
            302, "shift_r": 303, "shift_l": 304, "control_r": 305, "control_l":
            306, "super_l": 311, "super_r": 312, "print": 316}
         # Reverse mode.
         compliant_keymap_reverse = {
            8: "backspace", 9: "tab", 13: "return", 19: "pause", 32: "space",
            33: "exclam", 34: "quotedbl", 36: "dollar", 38: "ampersand", 39:
            "apostrophe", 40: "parenleft", 41: "parenright", 44: "comma", 45:
            "minus", 58: "colon", 59: "semicolon" ,60: "less", 61: "equal",
            94: "asciicircum", 95: "underscore", 97: "a", 98: "b", 99: "c",
            100: "d", 101: "e", 102: "f", 103: "g", 104: "h", 105: "i", 106:
            "j", 107: "k", 108: "l", 109: "m", 110: "n", 111: "o", 112: "p",
            113: "q", 114: "r", 115: "s", 116: "t", 117: "u", 118: "v", 119:
            "w", 120: "x", 121: "y", 122: "z", 127: "delete", 178:
            "twosuperior", 224: "agrave", 231: "ccedilla", 232: "egrave", 233:
            "eacute", 249: "ugrave", 256: "kp_0", 257: "kp_1", 258: "kp_2",
            259: "kp_3", 260: "kp_4", 261: "kp_5", 262: "kp_6", 263: "kp_home",
            263: "kp_7", 264: "kp_up", 264: "kp_8", 265: "kp_9", 266:
            "kp_decimal", 267:\
            "kp_divide", 268:"kp_multiply", 269:"kp_subtract", 270:"kp_add",
            271: "kp_enter", 273: "up", 274: "down", 275: "right", 276: "left",
            277: "insert", 278: "home", 279: "end", 280: "page_up", 281:
            "page_down", 300: "num_lock", 301: "caps_lock", 302: "scroll_lock",
            303: "shift_r", 304: "shift_l", 305: "control_r", 306: "control_l",
            311: "super_l", 312: "super_r", 316: "print"}

         def get_pressed(widget, event, key_pos, secondplayer=0):
            if device_radio.get_active() and widget.get_active() and \
               event.keyval: 
               # Getting the value (lower only).
               key_val = gtk.gdk.keyval_to_lower(event.keyval)
               ...
               if key_name in compliant_keymap.keys():
                  ...
                  else: self.p1key_int_vals[key_pos] = compliant_keymap[
                     key_name]

                  # Putting the key name (in an elegant form :p) as button
                  # label.
         ...
         self.widgets["config"]['libglpath'].connect("changed", set_path_icon,
            image)
         ...
         button.connect("clicked", self.file_select,
            _('Select the "%s" file.') % "libGL.so", self.widgets["config"]\
            ['libglpath'].get_text(), "libglpath")
         box2.pack_end(button, False)
         frame.add(box2)
         ...
         self.widgets["config"]['rominfoxml'].connect("changed", set_path_icon,
            image)
         ...
         if self.params["xgngeo"]['rominfos'] == "true":
            ...
            bouyaka(self.widgets["config"]['rominfos'], image,
             self.widgets["config"]\
               ['rominfoxml'], button)
         ...
         box2.pack_start(label)
         adjustment = gtk.Adjustment(float(self.params["xgngeo"]\
            ["historysize"]), 1, 20, 1)
   ...
   def config_write(self, widget, type, special=0, mamename=None):
      ...
      if type == 0:
         ...
         temp_param["raster"] = ("false","true")[self.widgets["config"]\
            ['raster'].get_active()]  # raster
         ...
         self.params["gngeo"]["libglpath"] = self.widgets["config"]\
            ['libglpath'].get_text() # libglpath
         ...
         self.params["xgngeo"]["rominfoxml"] = self.widgets["config"]\
         ...
         self.params["xgngeo"]["showavailableromsonly"] = ("false", "true")\
            [self.widgets["config"]['showavailableromsonly'].get_active()]
            # showavailableromsonly
         ...
         self.params["gngeo"]["bench"] = ("false","true")\
            [self.widgets["config"]['bench'].get_active()] # bench
      ...
      if letsWrite:  # We are now Ok to write into configuration file(s)...
         ...
         if special in (0, 1):
            ...
            if type == 0:
               ...
               self.emulator.set_path("gngeo", self.params["xgngeo"]\
               ["gngeopath"])
   ...
   def boot(self):
      if "--nobootcheck" in sys.argv:
         ...
         if not self.get_bios_presence(self.params["gngeo"]["biospath"]):
            error = 1
         # Does the ROM driver directory exist?
         elif not (os.path.isdir(self.params["gngeo"]["romrcdir"])): error = 1
         # Is the GnGeo executable present and returning correct version
         # values?
         elif not version or version[0][1] < 7: error = 1
