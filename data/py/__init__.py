...
class XGngeo:
	...
	def check_error(self):
		...
		"created. Anyway, correct values should be specified for the emulation to "\
		"work. Press OK to do so..."))
	...
	def romList(self,widget):
		def setRomTemp(treeview,path,view_column):
			...
			if self.params["xgngeo"]["rominfos"]=="true" and os.path.isfile(self.params["xgngeo"]["rominfoxml"]):
				...
				if self.romInfos.has_key(mamename):
					...
					if self.romInfos[mamename].has_key("size"): self.romInfosWidget["size"]\
					.set_text("%sMbit" % self.romInfos[mamename]["size"])
				else:
		...
		label = gtk.Label(_("Double-click on its name to select a ROM, then press "
		"Open to load it if available (blue background)."))
		label.set_line_wrap(True)
		label.set_justify(gtk.JUSTIFY_CENTER)
		...
		buttonShowAvailable.connect("toggled",refreshing_rom_list) #Connecting the toggled event (*after* potention button activation)
			...
			def updatePreview(*args):
				...
				if selection and os.path.isfile(selection): 
					...
					if self.params["temp"]["currdir_romscaninfos"].has_key(filename):
						...
						"<b>%s</b>."),"<b>%s</b>")[show_image=="true"]) % tuple[1])
	...
	def load_rom_from_history(self,fullname,path,availability):
		if os.path.exists(path): #ROM exists on file system, continuing...
			...
			message = _("Cannot continue: the ROM you wanted to load was not found on "\
			"the file system!")
	...
	def setting_current_rom(self,path,mamename,fullname):
		...
		if self.params["xgngeo"]["autoexecrom"]=="true": self.gngeo_exec() #Auto-executing the ROM...
	...
	def config(self,widget=None,type=0,firstrun=0,romspecific=0):
		def set_path_icon(widget,image,dir=0,special=None):
			...
			if not special:
				...
				if not (os.path.isfile("%s/neo-geo.rom" % path)\
				or os.path.isfile("%s/sp-s2.sp1" % path))\
				or not (os.path.isfile("%s/ng-sfix.rom" % path)\
				or os.path.isfile("%s/sfix.sfx" % path))\
				or not (os.path.isfile("%s/ng-lo.rom" % path)\
				or os.path.isfile("%s/000-lo.lo" % path)):
		...
		if firstrun: self.widgets["config"]["main_dialog"].connect("delete_event",
		self.quit)
		...
		if type==0:
			...
			_("Important path configuration"))
			...
			button.add(image)
			button.connect("clicked",self.file_select,_(
			"Select the main ROMs and BIOS directory."),self.widgets["config"]\
			['rompath'].get_text(),"rompath",1)
			...
			for line in pipe.readlines():
				...
				if plop:
					...
					self.widgets["config"]['blitter'].append_text((fullname,i18n_dict[ref])\
					[i18n_dict.has_key(ref)])
					blitter_list.append(ref)
					#Setting active the last selection.
					if ref==temp_param["blitter"]: self.widgets["config"]['blitter']\
					.set_active(i)
					i+=1
			pipe.close()
			...
			for line in pipe.readlines():
				...
				if plop:
					ref,fullname = plop.group(1).strip(),plop.group(2).strip()
					self.widgets["config"]['effect'].append_text((fullname,i18n_dict[ref])\
					[i18n_dict.has_key(ref)])
			...
			label = gtk.Label(_("To modify a key, click on the button under it's "\
			"icon, then push your new key. It works for keyboard only."))
			label.set_justify(gtk.JUSTIFY_CENTER)
			label.set_line_wrap(True)
			...
			def playerChanged(widget,justloaded=False):
				"""Performing some interface modifications when another player selection
				is changed."""
			...
			def editHotkeys(widget):
				...
				def genHotkeyRow(row,firstgen=False):
					...
					for butt in self.params["temp"]["hotkey_matrix_p%i" % player ][row]: 
						if not butt:
							break #Hot key's end has been reached, getting out the ``for".
			...
			self.widgets["config"]['previewimagedir'].connect("changed",set_path_icon,image,1)
			...
			_('Select the preview image directory.'),self.widgets["config"]\
			['previewimagedir'].get_text(),"previewimagedir",1)
			box2.pack_end(button,False)
			frame.add(box2)
			...
			_("Only show available ROMs in ROM list by default."))
		...
		if firstrun: button.connect("clicked",self.config_write,type,1)
		elif romspecific: button.connect("clicked",self.config_write,type,2,self.mamename)
				...
				if self.params["temp"].has_key("hotkey_matrix_p%i" % x):
					...
					for row in self.params["temp"]["hotkey_matrix_p%i" % x]:
						...
						if len(temp_param["p%ihotkey%i" % (x,i)]):
							temp_param["p%ihotkey%i" % (x,i)] = temp_param["p%ihotkey%i"\
							% (x,i)][:-1]
						i+=1
	...
	def main(self):
		...
		self.params["gngeo"]["rompath"],self.manual_rom_loading,0,{ _("All files") :
