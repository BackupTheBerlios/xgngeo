...
class Emulator:
   def __init__(self, *paths):
      self.paths = {"gngeo" : normalize_path(paths[0])}
      self.cmd = None
      self.romFullToMame = {}
      self.romMameToFull = {}
      self.romMameNames = []
   ...
   def get_gngeo_version(self, path=""):
      """Returning Gngeo version number as tuple and string (or None)."""
      pipe = os.popen("%s --version" % (normalize_path(path) or\
         self.paths['gngeo']), "r")
